// Package jugglermodels exposes shared lookup helpers for the
// per-user juggler model directory (~/.local/share/juggler/models).
// Both cmd/juggler and cmd/clown read this directory; consolidating
// the lookup keeps a single source of truth for the path layout.
package jugglermodels
...
func Dir() string {
	...
	return filepath.Join(home, ".local", "share", "juggler", "models")
}
