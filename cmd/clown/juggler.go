...
// passed --model. It lists `~/.local/share/juggler/models/*.gguf`
// and either returns one name or prints a diagnostic and returns
// a non-zero exit code:
//
//   - models dir empty / missing: hint the user to run `juggler download`
...
func pickJugglerModel() (string, int) {
	dir := jugglermodels.Dir()
	names, err := jugglermodels.List(dir)
	if err != nil {
		fmt.Fprintf(os.Stderr, "clown: listing juggler models in %s: %v\n", dir, err)
		return "", 1
	}
	if len(names) == 0 {
		fmt.Fprintf(os.Stderr,
			"clown: no juggler models found in %s.\n"+
				"       Run `juggler download <name>` first, or pass --model <path>.\n",
			dir)
		return "", 1
	}
	if len(names) == 1 {
	...
	if !isInteractiveTerminal() {
		fmt.Fprintf(os.Stderr,
			"clown: %d juggler models available in %s but stdin/stdout is not a terminal;\n"+
