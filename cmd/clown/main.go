...
func runWithFlags(flags parsedFlags) int {
	...
	// The dir lives under the launch staging root, which removes it when this
	// function returns. It is appended last so it cannot shadow a user- or
	// plugin-supplied dir. Only providers that actually consume the
	// synthesized dirs get them — see providerUsesPluginDirs.
	if providerUsesPluginDirs(flags.provider) && !flags.naked {
		if monitorDir, err := synthJobMonitorPluginDir(root, flags.identity.Key); err != nil {
	...
	// which machine and config model it is on up front, then the
	// fully-qualified session-identity requirement (clown#222) so
	// cross-session references never use a colliding bare clown-name. All
	// prompt-consuming providers read prompts.AppendFragments from this
	// struct, so this one mutation reaches each of them.
	lead := buildIdentityFragment()
	if host := hostIdentityFragment(); host != "" {
		lead += "\n\n" + host
	}
	lead += "\n\n" + fqIdentityFragment(flags.groupID, flags.clownName)
...
func runProvider(executor Executor, bound Command, logger *slog.Logger, flags parsedFlags) int {
	...
	if flags.printLaunchPlan {
		...
		// dirs — populating this is simply deferred, not blocked.
