...
// synthJugglerPluginDir writes a built-in plugin directory declaring
// `juggler mcp` as a stdioServers entry (docs/plans/2026-07-11-
// juggler-subagent-tool-design.md), and returns its path. The caller
// appends the path to the --plugin-dir set passed to Claude; the launch's
// staging root owns the directory, mirroring synthJobMonitorPluginDir's
// contract exactly — including that it gets its OWN directory under that
// root, never one shared with the job-monitor plugin. Returns ("", nil)
// when disabled (CLOWN_DISABLE_JUGGLER_MCP=1) or when
...
func synthJugglerPluginDir(root *staging.Root) (string, error) {
	...
	dir, err := root.Dir("clown-juggler-plugin-*")
	if err != nil {
		return "", err
	}
