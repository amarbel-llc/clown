...
//
//	juggler download <name>                                    (registry lookup)
//	juggler download <name> --url URL [--sha256 HEX] [--size N]
//
// --alias-style flags support both space and equals forms, matching
// juggler start. Returns the parsed name and overrides for url, sha,
...
func cmdDownload(args []string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "juggler: download: %v\n", err)
		fmt.Fprintln(os.Stderr, "usage: juggler download <name> [--url URL] [--sha256 HEX] [--size BYTES]")
		return 1
	}
	if name == "" {
		fmt.Fprintln(os.Stderr, "usage: juggler download <name> [--url URL] [--sha256 HEX] [--size BYTES]")
	...
	entries, err := jugglermodels.Registry()
	if err != nil {
		fmt.Fprintf(os.Stderr, "juggler: %v\n", err)
		return 1
	}
	entry, ok := jugglermodels.FindEntry(name, entries)
	switch {
	case !ok && urlOverride == "":
		...
		fmt.Fprintf(os.Stderr, "juggler: unknown model %q; available: %v\n", name, names)
		fmt.Fprintf(os.Stderr, "       to fetch from an arbitrary URL: juggler download %s --url <url> [--sha256 HEX] [--size BYTES]\n", name)
		return 1
	case !ok && urlOverride != "":
		// Ad-hoc download: synthesize an entry from the user's overrides.
		entry = jugglermodels.RegistryEntry{
			Name:        name,
			URL:         urlOverride,
			SHA256:      shaOverride,
			Size:        sizeOverride,
		...
		if shaOverride == "" {
			fmt.Fprintf(os.Stderr, "juggler: warning: --sha256 not provided; integrity will NOT be verified\n")
		}
	case ok && urlOverride != "":
		...
		fmt.Fprintf(os.Stderr, "juggler: --url override for registry entry %q\n", name)
		...
		if shaOverride != "" {
			...
			entry.SHA256 = ""
			fmt.Fprintln(os.Stderr, "juggler: warning: --sha256 not provided alongside --url; integrity will NOT be verified")
		}
		if sizeOverride > 0 {
	...
	go func() {
		_, err := jugglermodels.Download(context.Background(), entry, dir, progressCB)
		p.Send(doneMsg{err: err})
	}()

	result, err := p.Run()
	if err != nil {
		fmt.Fprintf(os.Stderr, "juggler: progress: %v\n", err)
		return 1
	}
	...
	fmt.Printf("juggler: downloaded %s to %s\n", name, dest)
	return 0
}
...
func downloadPlain(ctx context.Context, entry jugglermodels.RegistryEntry, dir string, w io.Writer) error {
	var written int64
	dest, err := jugglermodels.Download(ctx, entry, dir, func(n, _ int64) { written = n })
	if err != nil {
		return err
	}
	fmt.Fprintf(w, "juggler: downloaded %s: %d bytes to %s\n", entry.Name, written, dest)
	return nil
}
