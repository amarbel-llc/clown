...
// withClient dials juggler, runs fn with the connected client, and
...
func withClient(fn func(cli *rm.Client) int) int {
	cli, err := dialClient()
	if err != nil {
		fmt.Fprintf(os.Stderr, "juggler: %v\n", err)
		return 1
	}
	defer cli.Close()
	return fn(cli)
...
// cmdModels asks juggler for the list of installed GGUFs and prints
// each model's bare name, one per line — same shape as the pre-juggler
// jugglermodels.List scan, so existing shell pipelines keep working.
func cmdModels(cli *rm.Client) int {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	res, err := cli.ListAvailableModels(ctx)
	if err != nil {
		fmt.Fprintf(os.Stderr, "juggler: models: %v\n", err)
		return 1
	}
...
// cmdList asks juggler for the current set of live llama-server
...
func cmdList(cli *rm.Client) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "juggler: list: %v\n", err)
		return 1
	}
...
// instances to os.Stdout. Used by both `juggler list` and the no-alias
// path of `juggler status`, which want identical column layouts but
...
// human invoker isn't left guessing whether juggler is reachable.
...
func statusList(ctx context.Context, cli *rm.Client) int {
	res, err := cli.ListInstances(ctx)
	if err != nil {
		fmt.Fprintf(os.Stderr, "juggler: status: %v\n", err)
		return 1
	}
	if len(res.Instances) == 0 {
		fmt.Println("juggler: no instances running")
		return 0
	}
	if err := printInstanceTable(res.Instances); err != nil {
		fmt.Fprintf(os.Stderr, "juggler: status: %v\n", err)
...
// cmdStart parses argv for the new juggler-backed start command and
...
parse:
	for i := 0; i < len(args); i++ {
		...
		case a == "--alias":
			if i+1 >= len(args) {
				fmt.Fprintln(os.Stderr, "juggler: --alias requires an argument")
				return 1
			}
			alias = args[i+1]
			i++
		...
		case a == "--bind":
			if i+1 >= len(args) {
				fmt.Fprintln(os.Stderr, "juggler: --bind requires an argument")
				return 1
			}
			bind = args[i+1]
			i++
		...
		default:
			if model != "" {
				fmt.Fprintf(os.Stderr, "juggler: unexpected positional arg %q (model already set to %q)\n", a, model)
				return 1
			}
			model = a
		}
	...
	if model == "" {
		fmt.Fprintln(os.Stderr, "juggler: missing required model argument\nusage: juggler start <model> [--alias name] [--bind addr] [-- ...passthrough]")
		return 1
	}
	if alias == "" {
		alias = model
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "juggler: start: %v\n", err)
		return 1
	}
	in := res.Instance
	fmt.Printf("juggler: started %s at %s:%d (pid %d)\n", in.Alias, in.Bind, in.Port, in.PID)
	return 0
}
...
func cmdStop(cli *rm.Client, args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "juggler: missing alias\nusage: juggler stop <alias>")
		return 1
	}
	alias := args[0]
	...
	if err := cli.StopInstance(ctx, rm.StopInstanceParams{Alias: alias}); err != nil {
		fmt.Fprintf(os.Stderr, "juggler: stop: %v\n", err)
		return 1
	}
	fmt.Printf("juggler: stopped %s\n", alias)
	return 0
}
