...
func dialClient() (*rm.Client, error) {
	...
	if _, statErr := os.Stat(socket); errors.Is(statErr, fs.ErrNotExist) {
		printNotRunningHint()
		return nil, fmt.Errorf("juggler socket %s does not exist", socket)
	}
...
// printNotRunningHint writes the canonical "juggler is not running"
...
func printNotRunningHint() {
	fmt.Fprintf(
		os.Stderr,
		"juggler: juggler is not running.\n"+
