# End-to-end smoke test for the juggler control plane.
#
# Boots the control-plane daemon (`juggler daemon`, re-homed from
# `juggler daemon` when the job platform was extracted) on a temp
# Unix socket, points it at the Go fake-llama-server fixture via
# --llama-server, then drives the surface through the juggler CLI client
...
setup_file() {
  ...
  # The daemon (`juggler daemon`) and the juggler CLI client are the same
  ...
  local elapsed=0
  while [[ ! -S $JUGGLER_SOCKET && $elapsed -lt 50 ]]; do
    sleep 0.1
    elapsed=$((elapsed + 1))
  done
  if [[ ! -S $JUGGLER_SOCKET ]]; then
    echo "juggler daemon never bound socket; log:" >&2
    cat "$RM_DIR/rm.log" >&2
    return 1
  fi
}
...
@test "start launches an instance and prints its address" {
  run "$JUGGLER_BIN" start --alias e2e-one fake-model
  assert_success
  assert_line --regexp '^juggler: started e2e-one at 127\.0\.0\.1:[0-9]+ \(pid [0-9]+\)$'
}
...
@test "status for the alias prints key:value detail" {
  run "$JUGGLER_BIN" status e2e-one
  assert_success
...
@test "status for an unknown alias fails with not-found" {
  run "$JUGGLER_BIN" status no-such-alias
  assert_failure
  assert_output --partial 'not found'
}

@test "stop tears the instance down and list returns empty" {
  run "$JUGGLER_BIN" stop e2e-one
  assert_success
...
# Exercises that juggler correctly tracks two concurrent llama-server
# children through the real juggler CLI (not the in-process Go test).

@test "two instances can run concurrently" {
  run "$JUGGLER_BIN" start --alias multi-a fake-model
  assert_success
  assert_line --regexp '^juggler: started multi-a at 127\.0\.0\.1:[0-9]+ \(pid [0-9]+\)$'

  run "$JUGGLER_BIN" start --alias multi-b fake-model
  assert_success
  assert_line --regexp '^juggler: started multi-b at 127\.0\.0\.1:[0-9]+ \(pid [0-9]+\)$'
...
@test "two instances bind distinct ports" {
  run "$JUGGLER_BIN" status multi-a
  assert_success
  local port_a
  port_a=$(printf '%s\n' "${lines[@]}" | awk '/^port:/ {print $2}')
  [[ -n $port_a ]]

  run "$JUGGLER_BIN" status multi-b
  assert_success
  local port_b
  port_b=$(printf '%s\n' "${lines[@]}" | awk '/^port:/ {print $2}')
  [[ -n $port_b ]]
